                ...
                <template x-for="notif in notifications" :key="notif.id">
                    <div class="bg-brand-dark/50 rounded-lg p-3 border border-brand-light/5 flex items-start justify-between">
                        ...
                        <div class="ml-3 w-6 flex flex-col items-center gap-2">
                            ...
                            <button @click.stop="delete(notif.id)" class="text-xs text-red-400 hover:text-red-500">🗑️</button>
                        </div>
                    </div>
                </template>
            </div>
...
function notificationDropdown() {
    return {
        ...
        notifications: [],
        count: 0,
        toggle() { this.open = !this.open; if(this.open) this.fetchNotifications(); },
        close() { this.open = false; },
        fetchNotifications() {
            fetch('/notifications/list')
                .then(r => r.json())
                .then(data => { this.notifications = data.notifications; this.updateCount(); });
        },
        updateCount() {
        ...
        markAsRead(id) {
            ...
            }).then(r => r.json()).then(() => { this.fetchNotifications(); this.updateCount(); });
        },
        markAllAsRead() {
            fetch('/notifications/mark-all-read', { method: 'POST', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content') }})
                .then(r => r.json()).then(() => { this.fetchNotifications(); this.updateCount(); });
        },
        delete(id) {
            fetch('/notifications/delete', { method: 'DELETE', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content') }, body: JSON.stringify({ id })})
                .then(r => r.json()).then(() => { this.fetchNotifications(); this.updateCount(); });
        },
        timeAgo(dateStr) {
